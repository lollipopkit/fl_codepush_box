...
FunctionId（已实现于 `tool/fcb_kernel_manifest.dart`）：
...
完成情况：CLI、Go Fiber server、SQLite、Ed25519 签名、Rust updater 状态机、Flutter package FFI 调用全部落地。fcb init / release / patch / promote / check / install / rollback / mark-success / mark-failure 全套命令可用。e2e 测试覆盖 invalid signature 拒绝、bad patch blocklist。

Phase B：Android snapshot_replace backend ✅

完成情况：`vendor/flutter` fork 的 `flutter_main.cc::MaybeApplyFcbSnapshotReplace` 已 wire。bsdiff+zstd 实现于 `crates/fcb_core/src/diff.rs`，x86_64 emulator e2e 通过。

待办：
* arm64-v8a 真机走一轮 release → patch → restart → 触发 crash → 验证自动回滚。
* 文档化"snapshot_replace 仅用于 enterprise/internal，Play 不可用"。
...
完成情况：参见 `PLAN-now.md`。CLI release/patch 升级为基于 `fcb.yaml` 的可复现构建；release cache schema v2；build_info hard fail；linker plan；patch_report 三种状态全 audit trail；`--payload` 保留为手动逃生口。bytecode pipeline 改用 `flutter build bundle` 拿 Kernel。
...
* 补全 opcode：方法调用（call_static / call_dynamic / call_original）、字段读写、闭包捕获、字符串拼接/插值、is/as、try-catch、await。
* 编译器 Dart 化：扩展 `tool/fcb_kernel_manifest.dart`，从 Kernel AST 直接生成 binary BytecodeModule。
* 解释失败 fallback：异常路径自动转 `kDisabledBadPatch`，上报 `crash_rollback`。
* stack trace 映射：保留 FunctionId 与源码位置，DartError 在解释帧上可读。
* 退役 `crates/fcb_bytecode` 的编译职责，仅保留 schema 校验给 inspect 用。

验收：
* counter_app 真实业务级 patch（修改 widget tree、调用 setState、调用 plugin method channel）正确运行。
* 未修改函数继续跑原 AOT，interpreter ratio < 1%。
* 制造解释器抛错的 patch → 下次启动自动走原 AOT，server 收到 `crash_rollback` 事件。
