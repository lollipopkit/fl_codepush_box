...
1132 行，reader 为 1461 行，async helper 为 339 行，returning closure helper 为 234 行，
...
`return 'prefix ${await Future.value(value)}'` 这类 string interpolation 内 immediate await 子集、语句级 if-return 内 immediate await 子集，以及 `try { final x = await Future.value(...); if (...) return '$x ${await Future.value(...)}'; return ...; } catch (e) { return ...; }` 的 await-local tail if-return 子集，把
`Future<T> f() async { return value; }` / `Future<T> f() async { return await Future.value(value); }` / `Future<T> f() async { return '${await Future.value(value)}'; }` / `Future<T> f(bool b) async { if (b) return await Future.value(value); return other; }` / `Future<T> f() async { try { final x = await Future.value(value); if (ok) return '$x ${await Future.value(other)}'; return '$x fallback'; } catch (e) { return fallback; } }` 降级为
...
`tests/e2e/test_kernel_compile_from_plan.sh` 覆盖 `asyncLabel()`、`awaitedLabel()` 的语句级 if-return immediate await string interpolation 与 `awaitedLocalLabel()` 的 await-local tail if-return 进入 `interpret` 并生成
`_Future.value<String>` bytecode，`awaitedLocalLabel()` 同时包含 `TryBegin 0x61`、`StoreLocal 0x04`、`JumpIfFalse 0x31` 与 `StringConcat 0x42`；`value is String Function()` 与
...
| 文件 | 行数 | 角色 |
|------|------|------|
| `runtime/vm/fcb_patch_runtime.{h,cc}` / `runtime/vm/fcb_patch_runtime_value.cc` | 166 + 1486 + 109 | Value/InterpretResult 数据结构、PatchState 枚举、基础 opcode interpreter、ObjectPtr root visit、内部 bytecode closure context、bytecode closure 参数数量边界校验、bytecode closure 容器 materialization guard、interpreter-local try/throw handler stack、active handler 内 bytecode `CallStatic` 递归失败、VM helper 调用失败与 `AsType` mismatch 转 catch value、嵌套 bytecode 解释错误追加 FCB source location stack |
| `runtime/vm/fcb_patch_runtime_internal.h` / `runtime/vm/fcb_patch_runtime_vm.cc` | 78 + 1286 | VM ObjectPtr materialization、真实 Dart Map factory materialization、Dart instance field/dynamic/original/closure/type-check/new-object helper、bytecode closure named metadata helper、bytecode closure Dart materialization 显式拒绝边界、真实 Dart Error object catch value 输出、FCB patch source location stack trace metadata；standalone fallback helper |
| `runtime/vm/fcb_patch_runtime_loader.cc` | 723 | JSON/binary BytecodeModule 加载、source_map reader |
...
| P1 | `try_catch` / `throw` | `Throw 0x60` / `TryBegin 0x61` 已支持 interpreter-local 单 catch 子集；active handler 内的 bytecode `CallStatic` 递归失败和 VM helper 调用失败可进入 catch value，真实 Dart `Error` object 会作为 catch value 保留，`MakeClosure` missing target 这类解释器错误文本也可作为 catch value；`AsType` mismatch 在 active handler 下也可进入 catch value；未捕获的嵌套 bytecode closure throw 会保留 inner body 与 caller 两个 FCB source location；VM unwinder 与 Dart stack unwinding 对象级协作仍待实现 | 高 |
| P1 | `await` / `async_resume` | 无 `await` 的 async return、immediate `await Future.value(...)`、immediate await string interpolation、语句级 if-return immediate await、await-local 和 await-local tail if-return 已能生成 `_Future.value<T>`；真正挂起的 `await` continuation 与 `_FutureImpl` state machine 恢复仍未实现 | 极高 |
...
- 当前 compile-from-plan drill 为 39 个 interpreted functions、2 个 rejected functions、51 个 binary functions；`asyncLabel()` 的无 `await` async return、`awaitedLabel(bool enabled)` 的语句级 if-return immediate await string interpolation 和 `awaitedLocalLabel()` 的 await-local tail if-return 已进入 interpret，function type 与 record type 用例继续留在 reject，`logicalEscapingGreeting`、`ifElseEscapingGreeting`、`bodyLocalIfElseEscapingGreeting` 与 `branchLocalIfElseEscapingGreeting` 已进入 interpret。
...
- `tests/e2e/test_kernel_compile_from_plan.sh`：`asyncLabel()` 的 `Future<String> asyncLabel() async { return 'patched-async'; }` 与 `awaitedLabel(bool enabled)` 的 `if (enabled) return 'patched ${await Future.value('awaited')}'; return 'patched disabled';` 会生成 `NewObject 0x55`，constructor 常量为 `dart:async::class:_Future.value;types:String`，`awaitedLabel()` body 同时包含 `StringConcat 0x42` 与 `JumpIfFalse 0x31`；`awaitedLocalLabel()` 的 `try { final prefix = await Future.value(...); if (...) return '$prefix ${await Future.value(...)}'; return ...; } catch (e) { return ...; }` 生成 `TryBegin 0x61`、`StoreLocal 0x04`、`JumpIfFalse 0x31`、`StringConcat 0x42` 和 `_Future.value<String>`；本地 plan 会把它们放入 `interpret`；`value is String Function()` 与 `value is (String, int)` 会分别被 inventory 标记为 `function_type_unsupported`、`record_type_unsupported`，不生成 `bytecode_source`，本地 plan 会把它们放入 `reject`。
