...
4. **递归深度策略**:固定 `kMaxCallStaticDepth = 64` 已移除;默认 runaway guard 提升为
   ...
   返回业务 `DartException(StackOverflowError)`,entry bridge 会走 VM unwinder,不再 disable
...
- 递归:
  ...
  - 已完成:interpreter 入口接入 VM `OSThread::HasStackHeadroom()` C++ stack headroom guard;命中时
    返回 `DartException(StackOverflowError)` 并由 entry bridge 走 VM unwinder。测试用
    `force_vm_stack_overflow_for_testing` 覆盖该路径,保留真实 headroom 检查用于生产。
- Debugger:
