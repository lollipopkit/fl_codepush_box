...
结构化/fallback 解析,并支持 mapped item/key/value 形态)、async collection literal 中
...
双 pending-await local 后 switch spread、async `for`/`while`/`do-while` loop body 中的 `try/finally` / `try/catch`
...
subscription pause/resume 背压、sync*/async* 生成器内 switch expression OR pattern 与 lowered
switch statement OR pattern(`yield switch` 与 `case a || b: yield ...`),以及 sync*/async*
...
508/523/523;`make check-phase-e-host-evidence` 会检查该 summary
...
`fcb_kernel_switch_expr.dart`(344),switch statement lowering 拆到
`fcb_kernel_switch_statement_expr.dart`(750),collection literal / collection-for lowering
拆到 `fcb_kernel_collection_expr.dart`(574),collection append/runtime-for helper 拆到
`fcb_kernel_collection_append_expr.dart`(485))、`fcb_kernel_manifest.dart`(359,reader bundle
...
`fcb_kernel_manifest_control_compiler.dart`(475))、generator lowering 拆为
...
`fcb_kernel_generator_for_in_body_expr.dart`(337),`fcb_kernel_generator_expr.dart` 当前 779 行);
...
- `tests/e2e/test_kernel_compile_from_plan.sh`:通过。`target/fcb/kernel-compile-from-plan/summary.txt`
  记录 interpreted 508、reject 2、unchanged 13、module/binary function 523,并运行
