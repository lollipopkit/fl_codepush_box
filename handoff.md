...
- immediate async 子集已支持:
  ...
  - `AsyncReturn(0x63)` immediate `async_future`:有栈顶值时包装成 completed `_Future.value(value)`,空栈时包装成 completed `Future.value(null)`,并走正常 return/finally 路径。
...
- `ninja -C vendor/flutter/engine/src/out/host_debug_unopt_arm64 run_vm_tests`:阻塞于 GN regen,`vpython3 ... Returned 127`;新增 `FcbPatchRuntimeAsyncReturnCompletedFutureNull` 尚未由 rebuilt VM unit tests 验证。
...
- 根仓库为 `main...origin/main [ahead 16]`;embedded Dart 为 detached HEAD。
- `runtime/vm/fcb_patch_runtime_vm.cc` 约 1548 行,仍略超 1500,后续继续拆分。

**下一步**
1. 提交 embedded Dart,建议 `fix: support fcb async return null futures`。
2. 提交根仓库 plan/handoff,建议 `docs: update phase e async return null progress`。
