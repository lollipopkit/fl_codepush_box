...
- AOT generic static-call bridge 已支持 `TypeArguments + 4 user args` 五 raw slot 场景：`FcbAotStaticCall5` 调用 `FcbPatchStaticCallAot5`。
- 本轮新增 interpreter VM stack headroom guard：`runtime/vm/fcb_patch_runtime.cc` 在 `InterpretFunction` 入口检查 `OSThread::HasStackHeadroom()`，命中时 fail-closed 为 `PatchError`。
- `plans/phase_e_dart_vm.md` 已更新：C++ stack headroom guard 已完成；真正 Dart `StackOverflowError` unwinder 仍未完成。
...
- 尝试 rebuild `run_vm_tests` 失败在 depot_tools bootstrap：`vpython3` 缺 `.cipd_bin/vpython3`; `ensure_bootstrap` 暴露 mac-arm64 CIPD digest 缺失和 `curl: (18) Transferred a partial file`。

**当前状态**
- embedded Dart 有未提交改动：`runtime/vm/fcb_patch_runtime.cc`。
- 根仓库有未提交改动：`plans/phase_e_dart_vm.md`、`handoff.md`。
- `vendor/flutter` 外层未修改。
- `vendor/flutter/engine/src/flutter/third_party/depot_tools` 已清理 bootstrap 临时文件，状态干净。
- `runtime/vm/fcb_patch_runtime_vm.cc` 仍约 1548 行，后续应拆分。

**下一步**
1. 提交 embedded Dart，建议 `fix: guard fcb interpreter vm stack headroom`。
2. 提交根仓库文档，建议 `docs: update phase e stack guard progress`。
3. 修复/刷新 depot_tools bootstrap 后 rebuilt `run_vm_tests`，执行新增 AOT generic isolate tests。
