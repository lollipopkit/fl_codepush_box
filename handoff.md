...
- compile-from-plan 计数更新为 interpreted 461、reject 2、unchanged 13、module/binary function 476;reject 集合仍为 `isCallable:function_type_unsupported` 与 `isRecord:record_type_unsupported`。

**已验证**
- `tests/e2e/test_kernel_compile_from_plan.sh`:通过;两个 source async* VM filters 均 `Done`;summary 写入 461/2/13/476/476。
...
- `target/fcb/kernel-compile-from-plan/summary.txt` 已刷新到 461/2/13/476/476。
- 文件规模风险可控:release `07_switch_statements.dart` 196/1500,patch `07_switch_statements.dart` 315/1500,`fcb_kernel_switch_statement_expr.dart` 420/1500,`assert_plan_switch_statement_sources.py` 390/1500,`assert_module.py` 331/1500,`assert_binary.py` 543/1500。
