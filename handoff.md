...
- 已新增 Android Rust cross-build 入口：`packages/fcb_code_push/tool/build_android_native.sh <abi>` 使用 `cargo ndk` 构建 `fcb_updater` release `.so` 并复用 `prepare_android_prebuilt.sh` 放入 plugin `jniLibs`。
