...
FCB 有三个 patch backend。它们的合规边界、生命周期、维护策略截然不同。

| backend | 平台 | 执行引擎 | 合规 | 默认 |
|---|---|---|---|---|
| `bytecode` | 全平台(iOS/Android/桌面) | 自有 fork VM 内的 bytecode interpreter | ✅ 全平台合规 | ✅ |
| `dart_vm` | macOS/Linux/Windows | dart 官方 VM(kernel/JIT,release 不裁切) | 桌面自分发(非 iOS) | 桌面可选 |
| `snapshot_replace` | Android + 桌面 | 原生 `.so` 差分 | ❌ 非商店 | 关闭(需显式确认) |
...
## dart_vm —— 桌面官方 VM（macOS / Linux / Windows）

- **平台**:仅桌面三平台。**iOS 永不提供**(Apple 禁运行时机器码生成);Android 不在此 backend
  范围(走 bytecode 或 snapshot_replace)。
- **前提**:桌面 release 构建**保留官方 Dart VM 的 JIT/kernel 能力**(release 不裁切),使运行时可
  编译/执行新代码。桌面 release 默认是 AOT,本 backend 改以 JIT-runtime 引擎出包。
- **下发内容**:patch 是更新后的 kernel `.dill`(FCB 流水线本就从 `.dill` 起步)。
- **执行机制(推荐:整程序 kernel 替换)**:App 从 kernel blob 启动;patch 在下次启动时替换/覆盖
  `app.dill`,由官方 VM JIT 编译运行——概念上是"snapshot_replace 的 kernel 版":换一个 blob,VM
  自己跑。可承载任意代码改动(无 hot-reload 限制),回滚=退回内置 `.dill`(与其它 backend 一致)。
  - 备选机制:复用 VM hot-reload(`reloadSources`)在首帧前喂增量 kernel。patch 更小,但受
    hot-reload 限制(类层次/字段形状等改动不被允许),且部分失败会留下不一致 isolate 状态。
- **合规**:桌面自分发可用;Mac App Store(MAS)对可执行/可编译代码下发限制更严,MAS 分发应优先
  `bytecode`。
- **共同代价**:桌面二进制更大、JIT 预热启动开销;下发 kernel 比 AOT 更易被逆向。
- **状态**:设计已定(三 backend 模型);桌面 JIT 引擎出包与接线为 Phase X 工作(见
  `plans/phase_x_multiplatform.md`)。

## snapshot_replace —— enterprise/internal only（冻结）

- **平台**：Android(侧载/企业)+ 桌面三平台(自分发)。**iOS 永不提供**。
- **下发内容**：对原生 `libapp.so` 的 bsdiff+zstd 二进制差分,安装时重建 patched `.so`。
