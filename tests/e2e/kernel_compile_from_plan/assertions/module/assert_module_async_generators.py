...
def assert_generator_guarded_switch_function(module, name, async_kind, expected_constant, expected_yields):
    function = next(
        item for item in module["functions"] if item["name"].endswith(f"::{name}")
    )
    assert function.get("async_kind") == async_kind, function
    assert function.get("param_count") == 2, function
    assert function["code"].count(0x21) >= 2, function
    assert function["code"].count(0x31) >= 4, function
    assert function["code"].count(0x64) == expected_yields, function
    assert {"slot": 0, "name": "tier"} in function.get("debug_locals", []), function
    assert {"slot": 1, "name": "enabled"} in function.get("debug_locals", []), function
    assert any(
...
def assert_async_generators(module):
    ...
    assert_generator_guarded_switch_function(
        module,
