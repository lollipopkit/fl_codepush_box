...
def assert_stream_finally_await_cleanup_source(name, args, constants, min_move_next, min_cancel):
    source = patch_by_member.get(name, {}).get("bytecode_source", {})
    source_json = json.dumps(source)
    if (
        source.get("async_kind") != "async_star"
        ...
        raise SystemExit(f"expected {name} stream finally-await cleanup source, got {source}")
...
def assert_generator_guarded_switch_source(name, async_kind, expected_constants):
    source = patch_by_member.get(name, {}).get("bytecode_source", {})
    source_json = json.dumps(source)
    if (
        source.get("async_kind") != async_kind
        ...
        or '"bool": false' not in source_json
        or '"string": "gold"' not in source_json
        or '"string": "vip"' not in source_json
        or any(f'"string": "{constant}"' not in source_json for constant in expected_constants)
    ):
        raise SystemExit(f"expected {name} generator guarded switch source, got {source}")

assert_generator_guarded_switch_source(
    "syncGeneratedGuardedSwitchExpr",
    "sync_star",
    [
        "patched-iterable-guarded-switch-gold",
        "patched-iterable-guarded-switch-vip",
        "patched-iterable-guarded-switch-other",
    ],
)
assert_generator_guarded_switch_source(
    "syncGeneratedGuardedSwitchStatement",
    "sync_star",
    [
        "patched-iterable-guarded-switch-stmt-gold",
        "patched-iterable-guarded-switch-stmt-vip",
        "patched-iterable-guarded-switch-stmt-other",
    ],
)
assert_generator_guarded_switch_source(
    "asyncGeneratedGuardedSwitchExpr",
    "async_star",
    [
        "patched-stream-guarded-switch-gold",
        "patched-stream-guarded-switch-vip",
        "patched-stream-guarded-switch-other",
    ],
)
assert_generator_guarded_switch_source(
    "asyncGeneratedGuardedSwitchStatement",
    "async_star",
    [
        "patched-stream-guarded-switch-stmt-gold",
        "patched-stream-guarded-switch-stmt-vip",
        "patched-stream-guarded-switch-stmt-other",
    ],
)
...
assert_stream_finally_await_cleanup_source(
