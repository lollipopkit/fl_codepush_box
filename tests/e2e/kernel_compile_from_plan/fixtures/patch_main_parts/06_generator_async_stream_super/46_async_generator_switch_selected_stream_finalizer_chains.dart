Stream<String> asyncGeneratedSwitchSelectedYieldStarThenAwaitForFinallySuperChain(
  ...
  try {
    ...
    await for (final value in tail) {
      if (await stop) break;
      yield 'patched-stream-switch-selected-finalizer-body-$value';
    }
  } finally {
    yield* await cleanup;
  }
  ...
  try {
    await for (final outer in switch (await outerTier) {
      ...
      await for (final inner in switch (await innerTier) {
        ...
        yield 'patched-stream-switch-selected-finalizer-nested-$outer-$inner';
      }
    }
  } catch (e) {
    yield 'patched-stream-switch-selected-finalizer-caught-$e';
  } finally {
    await for (final value in cleanup) {
      yield 'patched-stream-switch-selected-finalizer-cleanup-$value';
    }
  }
}
