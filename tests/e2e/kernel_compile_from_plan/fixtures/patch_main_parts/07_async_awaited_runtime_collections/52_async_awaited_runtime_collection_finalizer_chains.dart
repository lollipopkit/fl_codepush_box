Future<List<String>> asyncListAwaitedRuntimeSourcesFinallyCleanupSuperChain(
  ...
  try {
    ...
    return ['patched-async-awaited-list-finally-disabled', ...await primary];
  } catch (e) {
    return [
      'patched-async-awaited-list-finally-caught-$e',
...
asyncMapAwaitedRuntimeSourcesFinallyCleanupSuperChain(
  ...
  try {
    return {
      switch (await tier) {
        'gold' => 'patched-async-awaited-map-finally-premium',
        _ => 'patched-async-awaited-map-finally-standard',
      }: await label,
    ...
    return {
      'patched-async-awaited-map-finally-caught': '$e',
      for (final entry in (await recovery).entries) entry.key: entry.value,
    };
  } finally {
    await {
      'patched-async-awaited-map-finally-cleanup': await label,
