Future<String> asyncNotAwaitIfTryFinallyTail(
  ...
  var out = 'patched-async-not-await-if-finally';
  if (!await ready) {
    try {
      out = '$out-body';
    } finally {
...
Future<String> asyncNotAwaitIfElseTryCatchFinallyTail(
  ...
  var out = 'patched-async-not-await-ifelse';
  if (!await ready) {
    try {
...
Future<String> asyncNotAwaitWhileTryFinallyLoop(
  ...
  var out = 'patched-async-not-await-while';
  var index = 0;
...
Future<String> asyncNotAwaitForTryCatchLoop(
  ...
  var out = 'patched-async-not-await-for';
...
Future<String> asyncNotAwaitDoWhileFinallyCondition(
  ...
  var out = 'patched-async-not-await-do';
  var index = 0;
  do {
    out = '$out-body-$index';
    final marker = await cleanup;
