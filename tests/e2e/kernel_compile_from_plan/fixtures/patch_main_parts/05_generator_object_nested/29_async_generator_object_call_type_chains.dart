Stream<List<String>> asyncGeneratedObjectDynamicTypeAwaitForCleanup(
  ...
  try {
    ...
    final user = User('patched-stream-object-dynamic-type-user', marker);
    ...
    await for (final value in body) {
      yield [
        'patched-stream-object-dynamic-type-body-$value',
        'patched-stream-object-dynamic-type-is-$isString',
        dynamicGreeter.surround(
          casted,
          prefix: 'patched-stream-object-dynamic-type-candidate-',
          suffix: box.value,
        ),
        for (final item in extra)
          dynamicGreeter.surround(
            item,
            prefix: 'patched-stream-object-dynamic-type-extra-',
            suffix: marker,
          ),
      ];
    }
    ...
    await for (final value in cleanup) {
      yield ['patched-stream-object-dynamic-type-cleanup-$value'];
    }
  }
}
...
Stream<Map<String, Object>> asyncGeneratedNamedObjectStaticYieldStarRecovery(
  ...
  try {
    ...
    yield {
      'head': Config(
        name: label('patched-stream-named-object-static-name'),
        label: marker,
      ),
      for (final entry in labels.entries)
        'patched-stream-named-object-static-${entry.key}': Config(
          name: entry.key,
    ...
    yield {'caught': 'patched-stream-named-object-static-caught-$e'};
    yield* recovery;
  } finally {
    yield* cleanup;
  }
  ...
  try {
    ...
    await for (final value in body) {
      yield dynamicGreeter.surround(
        value,
        prefix: 'patched-stream-object-call-await-for-',
        suffix: marker,
      );
    }
    yield* tail;
    ...
    await for (final value in cleanup) {
      yield 'patched-stream-object-call-await-for-cleanup-$value';
    }
  }
}
