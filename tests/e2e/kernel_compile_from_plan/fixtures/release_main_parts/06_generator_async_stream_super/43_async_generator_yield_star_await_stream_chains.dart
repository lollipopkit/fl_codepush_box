Stream<String> asyncGeneratedYieldStarAwaitStreamFutureSuperChain(
  ...
  try {
    ...
    await for (final value in body) {
      yield 'release-stream-yield-star-await-stream-body-$value';
    }
    if (await useTail) {
      yield* await tail;
    }
    ...
    await for (final value in cleanup) {
      yield 'release-stream-yield-star-await-stream-cleanup-$value';
    }
  }
}
...
Stream<String> asyncGeneratedYieldStarAwaitStreamFutureCatchFinallySuperChain(
  ...
  try {
    ...
    await for (final value in recovery) {
      yield 'release-stream-yield-star-await-stream-caught-$value-$e';
    }
  } finally {
    yield* await cleanup;
  }
