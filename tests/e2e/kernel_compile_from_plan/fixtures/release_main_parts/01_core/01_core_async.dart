...
Future<String> asyncIfElseBothTryFinallyAwaitTail(
  ...
  var out = 'base-ifelse-both-try-finally-await-tail';
  if (enabled) {
    try {
      final value = await ready;
      out = '$out-on-$value';
...
Future<String> asyncIfElseBothTryCatchFinallyAwaitTail(
  ...
  var out = 'base-ifelse-both-catch-finally-await-tail';
  if (enabled) {
    try {
      try {
        final value = await ready;
...
Future<String> asyncConditionalBothAwaitExpr(
  ...
  return enabled ? await fallback : await ready;
}
...
Future<String> asyncAwaitConditionConditionalBothAwaitExpr(
  ...
  return await enabled ? await fallback : await ready;
}
...
Future<String> asyncNestedConditionalAwaitExpr(
  ...
  return enabled ? (premium ? await fallback : await ready) : await disabled;
}
...
Future<String> asyncAwaitConditionNestedConditionalAwaitExpr(
  ...
  return await enabled
      ? (premium ? await fallback : await ready)
      : await disabled;
}
...
Future<String> asyncIfLogicalAndAwaitTail(
  ...
  var out = 'base-if-logical-and-await';
...
Future<String> asyncIfElseLogicalOrAwaitTail(
  ...
  var out = 'base-ifelse-logical-or-await';
...
Future<String> asyncIfNestedLogicalAwaitReturn(
  ...
  if (enabled && (await fallback || await ready)) {
    return 'base-if-nested-logical-await-on';
  }
  return 'base-if-nested-logical-await-off';
}
...
Future<String> asyncWhileLogicalAwaitCondition(
  ...
  var i = 0;
  var out = 'base-while-logical-await-condition';
